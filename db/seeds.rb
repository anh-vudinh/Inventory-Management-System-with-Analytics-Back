...
10.times do
puts "new company"
comp = Company.new(name: "co#{rand(1..25)}", location: "#{rand(1000..9999)} SomeStreet, Pearland, TX, #{rand(10000..99999)}", description: "test company description", parent_id: assign_parent_id, is_parent: assign_is_parent, is_active: [true, false].sample, established: Date.today.advance(days: rand(-1000..1000)), structure: structures.sample, organization: organizations.sample, industry: industries.sample)
    if comp.valid?
        comp.save
...
def assign_parent_id
    possible_parent = rand(0..5)
    if possible_parent == 0
        assign_is_parent(true)
        0
    else
        assign_is_parent(false)
        possible_parent
    end
end

def assign_is_parent(boolean)
    boolean
end
